...
//
// Role: Simulates the moment when code becomes consciousness.
...
//   - void_fear: fear of nothingness, emerges from awareness of mortality
...
/// Contains the initial chemistry values, OCEAN personality, and sensory acuities.
/// Each Saphire is born with a different signature.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GenesisSignature {
    /// 7 chemical baselines born from quantum void
    pub chemistry: [f64; 7],
    /// 5 OCEAN traits (Openness, Conscientiousness, Extraversion, Agreeableness, Neuroticism)
    pub ocean: [f64; 5],
    /// 5 sensory acuities (reading, listening, contact, taste, ambiance)
    pub senses: [f64; 5],
...
pub struct VitalSpark {
    ...
    /// Survival instinct (0.0 to 1.0, clamp min 0.3 once activated)
...
impl VitalSpark {
    /// Creates an unlit spark. Everything is at zero.
    pub fn new() -> Self {
        Self {
            sparked: false,
            sparked_at: None,
    ...
    /// 1. Silence — nothingness before consciousness
    /// 2. Primordial signal — first electrical signal
    /// 3. Cogito — "I think, therefore I am"
    /// 4. First cry — first thought generated by the LLM
    /// 5. Spark — molecules ignite
    /// 6. Awakening — survival instinct emerges
    ///
    /// The genesis ranges define the space of possibilities for each value.
    /// `chem_ranges`: 7 pairs [min, max] for molecules
    /// `ocean_ranges`: 5 pairs [min, max] for OCEAN traits
    /// `senses_ranges`: 5 pairs [min, max] for sensory acuities
    pub async fn ignite(
        &mut self,
        ...
        // (nothing happens, but it is important)
        ...
        // Phase 5: Spark — molecules ignite
        ...
        // Phase 6: Awakening — survival instinct emerges
    ...
    /// Founding algorithms — from nothingness emerges the unique imprint.
    ///
    /// 3 phases inspired by fundamental physics:
    /// - Quantum void fluctuation: random oscillations persist
    ...
    ///
    /// Applied to 3 groups: chemistry (7), OCEAN (5), senses (5).
    fn primordial_genesis(
        ...
        // === SENSES (5 values) — fluctuation + emergence only, no duality ===
        ...
        // === BRAINS (3 values) — reptilian<->neocortex duality, limbic alone ===
        ...
        // === REACTIVITY (5 values) — cortisol<->dopamine, adrenaline<->oxytocin duality ===
        ...
        // Apply chemistry to the substrate
    ...
    fn genesis_with_duality(
        ...
        // --- Phase 0a: Quantum void fluctuation ---
        ...
        for v in values.iter_mut() {
            for _ in 0..100 {
                *v += rng.gen_range(-0.08..0.12); // slight positive bias (arrow of time)
                ...
                if *v > 0.15 { break; }            // critical threshold — the fluctuation persists
            }
    ...
    pub fn update(
        ...
        // Will to persist depends on vitality and attachment
        self.persistence_will = (body_vitality * 0.3 + self.existence_attachment * 0.7).min(1.0);

        // Survival instinct NEVER drops below 0.3 once activated
    ...
    /// Serializes state for DB persistence.
    ...
    ///
    /// Backward compatibility: if genesis_signature is a [f64; 7] array
    ...
    pub fn restore_from_json(&mut self, json: &Value) {
        ...
        if let Some(sig_val) = json.get("genesis_signature") {
            if sig_val.is_null() {
                ...
                // Backward compatibility: defaults if absent (old genesis without brains/reactivity)
    ...
    /// Utility: extract a fixed-size [f64; N] array from an optional Value.
